...
const PostDetailView: React.FC<PostDetailViewProps> = ({ notification, onBack, onProfileClick }) => {
  ...
  const fetchComments = useCallback(async (idOfPost: string) => {
      ...
      try {
          const commentsRes = await fetch(`${API_BASE_URL}/api/v1/posts/${idOfPost}/comments`, { headers: { 'Authorization': `Bearer ${token}` } });
          if (commentsRes.ok) {
              const commentsData = await commentsRes.json();
              const rawComments = Array.isArray(commentsData) ? commentsData : (commentsData.comments || []);
              const processedComments = rawComments.map(processApiComment).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    const fetchPostDetails = async () => {
      ...
      try {
        ...
        if (postRes.ok) {
           ...
           // Only fetch comments if they weren't in the cache
  ...
  const handleSendComment = async (isReplyDrawer: boolean = false) => {
    ...
    if (isReply && parentId) {
        ...
        // Note: viewingRepliesFor will be updated by the useEffect, but we can set it here for instant feedback too
    ...
    try {
        ...
        if (response.ok) {
            ...
            const serverComment = data.comment || data.data || data;

            if (serverComment && (serverComment._id || serverComment.id)) {
                const realComment = processApiComment(serverComment);
                if (!realComment.user || !realComment.user.avatar) realComment.user = optimisticComment.user;
                ...
                if (isReply && parentId) {
                     ...
                     setComments(prev => {
                         const updated = swapReplyId(prev);
                         updateCache(post, updated, isLiked, localLikesCount);
                         return updated;
                     });
                } else {
                    setComments(prev => {
                        const updated = prev.map(c => c._id === tempId ? realComment : c);
                        updateCache(post, updated, isLiked, localLikesCount);
                        return updated;
                    });
                }
  ...
  const handleCommentLike = async (commentId: string, parentId?: string) => {
    ...
    try {
        ...
        const endpoint = targetParentId 
            ? `${API_BASE_URL}/api/v1/posts/${post?.id}/comment/${targetParentId}/reply/${commentId}/like`
  ...
  const confirmDeleteComment = async () => {
    ...
    try {
        ...
        const endpoint = parentComment
          ? `${API_BASE_URL}/api/v1/posts/${post.id}/comment/${parentComment._id}/reply/${idToDelete}`
          : `${API_BASE_URL}/api/v1/posts/${post.id}/comment/${idToDelete}`;
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-white flex flex-col w-full h-full overflow-x-hidden animate-in slide-in-from-bottom duration-300">
      
      {/* REPLIES DRAWER */}
      <div className={`fixed inset-y-0 right-0 w-full md:w-[480px] bg-white z-[160] transform transition-transform duration-300 ease-in-out flex flex-col ${viewingRepliesFor ? 'translate-x-0 shadow-2xl' : (language === 'ar' ? '-translate-x-full' : 'translate-x-full')}`}>
