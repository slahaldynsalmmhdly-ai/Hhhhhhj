...
import { 
  ...
  ArrowRight, MoreHorizontal, Loader2, Bell, Check, Trash2, CheckCircle
} from 'lucide-react';
...
interface APINotification {
  ...
  // Allow for both populated object or string ID
  post?: {
    _id: string;
    image?: string;
  } | string;
...
let globalDisplayedCount = 10; // Persist how many items user has loaded
let globalScrollPosition = 0;  // Persist scroll position
...
const NotificationsView: React.FC<NotificationsViewProps> = ({ onClose, onNotificationClick, onProfileClick }) => {
  ...
  const NEXT_BATCH = 10; // Increased load batch for better UX
  ...
  // Use globalDisplayedCount instead of fixed initial batch
  ...
  // Only show loading spinner if we haven't loaded data at least once in this session
  const [loading, setLoading] = useState(!hasLoadedNotificationsOnce);
  
  const [loadingMore, setLoadingMore] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  
  // Single Item Menu State
  ...
  }, [displayedNotifications]); // Re-apply when list renders
  ...
  // Reusable function to fetch notifications from server
  const fetchNotifications = useCallback(async (isBackgroundUpdate = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let rawNotifications = Array.isArray(data) ? data : [];
        ...
        // Update displayed list respecting the globalDisplayedCount
        // ensuring we don't shrink the list if the user has scrolled down
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // Save scroll position continuously (optional, but safe)
  ...
  const loadMoreNotifications = async () => {
    ...
    // Update global counter so next visit starts with this amount
  ...
  const markAsRead = async (notifId: string, refreshFromServer = true) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/users/me/notifications/${notifId}/read`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const markAllAsRead = async () => {
    ...
    try {
       ...
       await Promise.all(unreadNotifications.map(n => 
          fetch(`${API_BASE_URL}/api/v1/users/me/notifications/${n._id}/read`, {
            method: 'PUT',
            headers: { 'Authorization': `Bearer ${token}` }
          })
       ));
  ...
  const handleDeleteNotification = async () => {
      ...
      try {
          const response = await fetch(`${API_BASE_URL}/api/v1/users/me/notifications/${notificationToDelete}`, {
              method: 'DELETE',
              headers: { 'Authorization': `Bearer ${token}` }
          });
  ...
  const getNotificationMessage = (notif: APINotification) => {
    ...
    if (type.includes('follow') || type === 'new_follower') {
        return `${senderName} ${t('notif_follow')}`;
    }
    if (type.includes('like') || type === 'post_like') {
        return `${senderName} ${t('notif_like_post')}`;
    }
    if (type.includes('comment') || type === 'post_comment') {
        return `${senderName} ${t('notif_comment_post')}`;
    }
    if (type.includes('reply') || type === 'post_reply') {
        return `${senderName} ${t('notif_reply_post')}`;
    }
    if (type === 'short_like') {
        return `${senderName} ${t('notif_like_short')}`;
    }
    if (type === 'short_comment') {
        return `${senderName} ${t('notif_comment_short')}`;
    }
    if (notif.message && notif.message.toLowerCase().includes('started following')) {
       return `${senderName} ${t('notif_follow')}`;
    }
    return notif.message || t('notif_general');
  };
  ...
  const handleItemClick = (notif: APINotification) => {
    ...
    if (notif.post) {
       ...
       if (typeof notif.post === 'object') {
           targetId = notif.post._id || (notif.post as any).id;
           imagePreview = notif.post.image 
              ? (notif.post.image.startsWith('http') ? notif.post.image : `${API_BASE_URL}${notif.post.image}`)
              : null;
       } else if (typeof notif.post === 'string') {
           targetId = notif.post;
       }
    } else if (notif.short) {
       category = 'video';
       if (typeof notif.short === 'object') {
           targetId = notif.short._id || (notif.short as any).id;
           imagePreview = notif.short.thumbnail
              ? (notif.short.thumbnail.startsWith('http') ? notif.short.thumbnail : `${API_BASE_URL}${notif.short.thumbnail}`)
              : null;
       } else if (typeof notif.short === 'string') {
           targetId = notif.short;
       }
    }
    ...
    if (notif.type.toLowerCase().includes('follow') || notif.type === 'new_follower') {
       return; 
    }
