...
const CreatePostModal: React.FC<CreatePostModalProps> = ({ onClose, onPostSubmit }) => {
  ...
  const handleFinalPost = async () => {
    ...
    try {
      ...
      let finalTitle = '';
      let finalPostType = undefined;

      // CRITICAL FIX: Ensure 'title' and 'postType' are Arabic strings 
      // matching what JobsView.tsx expects ('ابحث عن وظيفة' or 'ابحث عن موظفين')
      if (isJobPost) {
          if (jobType === 'seeker') {
              finalTitle = 'ابحث عن وظيفة';
              finalPostType = 'ابحث عن وظيفة';
          } else {
              finalTitle = 'ابحث عن موظفين';
              finalPostType = 'ابحث عن موظفين';
          }
      }
      ...
      const postPayload = {
        ...
        city: scope === 'local' ? (selectedCity || t('all_cities')) : null,
    ...
    <div className="fixed inset-0 z-[100] bg-white dark:bg-black animate-in slide-in-from-left duration-300 flex flex-col" style={{ willChange: 'transform' }}>
             ...
             <div className="animate-in fade-in slide-in-from-top-1 duration-200 space-y-3">
                ...
                >
                   <span className={selectedCountry ? "text-gray-900 dark:text-white" : "text-gray-400"}>
                     {selectedCountry || t('location_select_country')}
                   </span>
                ...
                >
                   <span className={selectedCity ? "text-gray-900 dark:text-white" : "text-gray-400"}>
                     {selectedCity === 'All Cities' ? t('location_all_cities') : (selectedCity || t('location_select_city_opt'))}
                   </span>
                   <ChevronDown size={16} className="text-gray-400" />
                </div>
          ...
          >
             <div className="flex items-center gap-3">
                ...
                <div>
                   ...
                   <p className="text-[10px] text-gray-400">{t('premium_subtitle')}</p>
                </div>
             </div>
               ...
               <div className="flex-1 overflow-y-auto no-scrollbar p-2">
                 {ARAB_LOCATIONS.map((loc) => (
                    ...
                    >
                      ...
                      {selectedCountry === loc.country && <Check size={16} className="text-blue-600" />}
                    </button>
                 ))}
               </div>
            </div>
       ...
       {isCityDrawerOpen && selectedCountry && (
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#121212] z-[130] rounded-t-3xl h-[60vh] flex flex-col animate-in slide-in-from-bottom duration-300">
               <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
                  <span className="font-bold text-gray-800 dark:text-white">{t('location_cities_in')} {selectedCountry}</span>
               ...
               <div className="flex-1 overflow-y-auto no-scrollbar p-2">
                 ...
                 <button 
                  ...
                  >
                    ...
                    {selectedCity === 'All Cities' && <Check size={16} className="text-blue-600" />}
                  </button>

                 {ARAB_LOCATIONS.find(l => l.country === selectedCountry)?.cities.map((city) => (
                    <button 
                      key={city}
                      onClick={() => {
                    ...
                    >
                      <span>{city}</span>
                      {selectedCity === city && <Check size={16} className="text-blue-600" />}
                    </button>
                 ))}
               </div>
            </div>
