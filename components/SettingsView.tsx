...
const SettingsView: React.FC<SettingsViewProps> = ({ onClose, onProfileClick, onLogout, isDarkMode, onToggleDarkMode }) => {
  ...
  const renderContentModal = () => {
    ...
    if (activeModal === 'wallet') {
        title = t('settings_subscriptions'); // Using wallet title from context
        content = (
            <div className="space-y-6">
                
                {/* Bank Card Simulation */}
                <div className="w-full aspect-[1.586] rounded-2xl bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white p-6 relative overflow-hidden shadow-2xl border border-white/10">
                    ...
                    <div className="absolute bottom-0 left-0 w-48 h-48 bg-blue-600/20 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3"></div>
                    
                    <div className="relative z-10 flex flex-col justify-between h-full">
                        <div className="flex justify-between items-start">
                            <Cpu size={32} className="text-yellow-200 opacity-90 drop-shadow-sm" />
                            <Nfc size={24} className="text-white/60 rotate-90" />
                        </div>

                        <div className="flex flex-col items-center justify-center my-2">
                            <span className="text-xs text-white/60 uppercase tracking-widest mb-1">{t('wallet_balance')}</span>
                            <div className="flex items-center gap-1">
                                <span className="text-4xl font-bold tracking-tight text-white drop-shadow-md font-mono">{walletBalance.toLocaleString()}</span>
                                <span className="text-sm font-medium text-yellow-400 mt-3">{t('wallet_currency')}</span>
                            </div>
                        </div>

                        <div className="flex justify-between items-end">
                            <div>
                                <p className="text-sm font-bold tracking-wide shadow-black drop-shadow-sm uppercase truncate max-w-[140px]">{userName}</p>
                            </div>
                            <p className="text-lg font-black tracking-widest text-white/80">{t('app_name')}</p>
                        </div>
                    </div>
                </div>
                ...
                {/* Purchase Options */}
                <div>
                    <h3 className="text-sm font-bold text-gray-800 mb-3 px-1">{t('wallet_buy')}</h3>
                    <div className="flex flex-col gap-3">
                        {[
                            { amount: 1, price: '10', color: 'bg-blue-50 border-blue-200 text-blue-800', iconColor: 'text-blue-600' },
                            { amount: 10, price: '100', color: 'bg-purple-50 border-purple-200 text-purple-800', iconColor: 'text-purple-600' },
                            { amount: 50, price: '500', color: 'bg-orange-50 border-orange-200 text-orange-800', iconColor: 'text-orange-600' },
                            { amount: 100, price: '1000', color: 'bg-emerald-50 border-emerald-200 text-emerald-800', iconColor: 'text-emerald-600' }
                        ].map((pkg, idx) => (
                            <button 
                                key={idx}
                                onClick={() => handleBuyCoins(pkg.amount)}
                                className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all active:scale-95 hover:shadow-md ${pkg.color}`}
                            >
                                <div className="flex items-center gap-4">
                                    <div className="bg-white p-3 rounded-full shadow-sm">
                                        <Coins size={24} className={pkg.iconColor} />
                                    </div>
                                    <div className="text-right">
                                        <span className="block text-xl font-black leading-none">{pkg.amount}</span>
                                        <span className="text-xs font-bold opacity-70">{t('wallet_gold')}</span>
                                    </div>
                                </div>

                                <div className="bg-white/60 px-4 py-2 rounded-xl text-sm font-bold shadow-sm backdrop-blur-sm">
                                    {pkg.price}
                                </div>
                            </button>
                        ))}
        ...
        content = (
            <div className="space-y-6">
                <div className="bg-orange-50 border-r-4 border-orange-500 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-orange-700 leading-relaxed font-bold whitespace-pre-line">
                        {t('warning_strict_desc')}
                    </p>
                </div>
            </div>
        );
        ...
        content = (
            <div className="flex flex-col items-center text-center space-y-6 pt-4">
                ...
                <div>
                    ...
                    <p className="text-gray-500 font-bold text-sm">v1.0.2</p>
                </div>
                ...
                {activeModal !== 'language_select' && (
                    <div className="p-5 border-t border-gray-100 bg-white pb-safe rounded-b-3xl">
                        {isReportModal ? (
                            ...
                            >
                                {activeModal === 'wallet' ? t('close') : t('understood')}
                            </button>
                        )}
                    </div>
                )}
