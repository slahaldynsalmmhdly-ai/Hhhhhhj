...
const SuggestedCard: React.FC<{ item: SuggestedItem; type: string; onProfileClick?: (id: string) => void }> = ({ item, type, onProfileClick }) => {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      ...
      if (item.id === currentUserId || !token) return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Update Cache
          localStorage.setItem(`follow_status_${item.id}`, JSON.stringify(data.isFollowing));
        }
      } catch (error) {
        console.error("Failed to sync follow status in suggested list", error);
      }
    };
  ...
  // 3. Handle Toggle (Instant / Optimistic)
